...
class BarnowlHci extends EventEmitter {
   ...
   */
  close() {
    let numberOfListeners = this.listeners.length;
    let numberOfClosedListeners = 0;

    this.listeners.forEach((listener) => {
      listener.close(() => {
        if(++numberOfClosedListeners === numberOfListeners) {
          console.log('barnowl-hci: closed listeners, exiting...');
          process.exit(0);
        }
      });
    });
