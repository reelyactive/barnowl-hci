...
function openHciSocket(path, callback) {
  ...
  //setEventMask(bluetoothHciSocket);
  //setLeEventMask(bluetoothHciSocket);
...
function setEventMask(socket) {
  ...
  const eventMask = Buffer.from('fffffbff07f8bf3d', 'hex'); // TODO: check
...
function setLeEventMask(socket) {
  const cmd = Buffer.alloc(12);
  const leEventMask = Buffer.from('1fff000000000000', 'hex'); // TODO: check
