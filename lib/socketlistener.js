...
class SocketListener {
  ...
  constructor(options) {
    ...
    this.origin = 'socket';
    if(this.deviceId) {
      this.origin = 'hci' + this.deviceId;
    }

    openHciSocket(self, (err, socket) => {
      if(err) {
        let device = self.deviceId ? 'hci' + self.deviceId : 'hciX';
        return console.log('barnowl-hci (' + device + '): ' + err.message);
      }
      else {
        self.socket = socket;
  ...
  /**
   * Exit gracefully, disabling scanning and stopping the socket.
   * @param {function} callback The function to call on completion.
   */
  close(callback) {
    setScanEnable(this.socket, false, true);
    this.socket.stop();
    return callback();
  }
...
/**
 * Open the HCI socket based on the given deviceId and driver.
 * @param {SocketListener} instance The SocketListener instance.
 * @param {function} callback The function to call on completion.
 */
function openHciSocket(instance, callback) {
  let bluetoothHciSocket;
  switch(instance.driver) {
    case 'uart':
    case 'usb':
    case 'native':
      const SpecificBluetoothHciSocket =
                                BluetoothHciSocket.loadDriver(instance.driver);
      bluetoothHciSocket = new SpecificBluetoothHciSocket();
      break;
    default:
      bluetoothHciSocket = new BluetoothHciSocket();
  }

  try {
    if(Number.isInteger(instance.deviceId)) {
      bluetoothHciSocket.bindRaw(instance.deviceId);
    }
    else {
    ...
    return callback(error, bluetoothHciSocket);
  }
  ...
  callback(null, bluetoothHciSocket);
}
